...
def jes_es(monkeypatch, mock_client):
    # patch Elasticsearch constructor to return mock_client
    with patch(
        "oxygent.databases.db_es.jes_es.Elasticsearch", return_value=mock_client
    ):
...
async def test_create_index_new(jes_es, mock_client):
    ...
    mock_client.indices.create.assert_called_once_with(index="new_index", body=body)
...
async def test_search_doc(jes_es, mock_client):
    ...
    assert "hits" in res
    mock_client.search.assert_called_once_with(index="idx", body=query)
...
async def test_exists_doc(jes_es, mock_client):
    ...
    assert res is True
    mock_client.exists.assert_called_once_with(index="idx", id="1")
